...
class Bot(models.Model):
    ...
    status = models.CharField(max_length=64, null=True,
                              blank=True, choices=BOT_STATUS_CHOICES)
...
class Trash(models.Model):
    ...
    degradable_trash = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
    non_degradable_trash = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True)
...
class LastLocation(models.Model):
    ...
    image = models.ImageField(
        upload_to='geo-locations/', null=True, blank=True)
    time_stamp = models.DateTimeField(null=True, blank=True)

    def __str__(self):
        return f'{self.bot} - ({self.lat}, {self.lon}) at {self.time_stamp}'
...
class Obstacles(models.Model):
    ...
    status = models.CharField(
        max_length=64, choices=OBSTACLE_STATUS_CHOICES, null=True, blank=True)
    slug = models.CharField(max_length=128, null=True, blank=True)
...
class LastDump(models.Model):
    ...
    dump_centre = models.ForeignKey(
        'TrashCollectionCenter', on_delete=models.SET_NULL, null=True)
